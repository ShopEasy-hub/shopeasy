...
export function WarehousesUnified({ appState, onNavigate }: WarehousesUnifiedProps) {
  ...
  // Helper functions for warehouse limits
  function canAddWarehouseLocal(): boolean {
    const { canAddWarehouse, WAREHOUSE_LIMITS } = require('../lib/permissions');
    const currentPlan = appState.subscriptionPlan || 'starter';
    const limit = WAREHOUSE_LIMITS[currentPlan] || 0;
    
    // If limit is 0, warehouse access is not available
    if (limit === 0) return false;
    
    return canAddWarehouse(currentPlan, warehouses.length);
  }

  function getWarehouseLimitMessageLocal(): string {
    const { getWarehouseLimitMessage } = require('../lib/permissions');
    return getWarehouseLimitMessage(appState.subscriptionPlan || 'starter', warehouses.length);
  }
        ...
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between mb-4">
            ...
            {activeTab === 'list' && (
              <Button 
                onClick={() => {
                  if (canAddWarehouseLocal()) {
                    setShowAddWarehouse(true);
                  } else {
                    alert(`Warehouse limit reached!\n\n${getWarehouseLimitMessageLocal()}\n\nPlease upgrade your plan to add more warehouses.`);
                  }
                }}
                className="gap-2"
                disabled={!canAddWarehouseLocal()}
              >
