...
export default function ProductHistory({ appState, onNavigate }: ProductHistoryProps) {
  ...
  useEffect(() => {
    if (isAuthorized && appState.organizationId) {
      loadProducts();
      loadBranches();
      loadCashiers();
    }
  }, [appState.organizationId, isAuthorized]);
  ...
  const loadProducts = async () => {
    try {
      const { data, error } = await supabase
        ...
        .eq('organization_id', appState.organizationId)
        .order('name');
  ...
  const loadProductHistory = async () => {
    ...
    try {
      ...
      let query = supabase
        ...
        .eq('sales.organization_id', appState.organizationId); // CRITICAL: Filter by organization
      ...
      const history: SaleHistory[] = (data || []).map((item: any) => {
        ...
        return {
          ...
          product_name: item.name || selectedProduct.name, // Fallback to selected product name
  ...
  const calculateStats = (history: SaleHistory[]) => {
    ...
    const topBranch = Object.keys(branchSales).reduce((a, b) => branchSales[a] > branchSales[b] ? a : b, '');
    ...
    const topCashier = Object.keys(cashierSales).reduce((a, b) => cashierSales[a] > cashierSales[b] ? a : b, '');
                  ...
                  <CardContent className="pt-6">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-2xl font-bold">{productStats.total_sales}</p>
                      </div>
                  ...
                  <CardContent className="pt-6">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-2xl font-bold">{productStats.total_quantity}</p>
                      </div>
                                ...
                                <TableRow className="bg-gray-50">
                                  <TableCell colSpan={10}>
                                    <div className="p-4 grid grid-cols-2 md:grid-cols-4 gap-4">
                                      ...
                                      <div>
                                        <p className="text-sm font-medium text-gray-600">Product SKU</p>
                                        <p className="text-sm">{sale.product_sku}</p>
                                      </div>
                                      <div>
                                        <p className="text-sm font-medium text-gray-600">Sale Total</p>
