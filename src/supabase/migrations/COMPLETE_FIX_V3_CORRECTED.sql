-- =====================================================
-- COMPLETE FIX V3 - CORRECTED EXCEPTION HANDLING
...
EXCEPTION
    ...
    WHEN OTHERS THEN
        RAISE NOTICE '⚠️ Constraint may already exist: %', SQLERRM;
END $$;
...
EXCEPTION
    ...
    WHEN OTHERS THEN
        RAISE NOTICE '⚠️ Branch index may already exist: %', SQLERRM;
END $$;
...
EXCEPTION
    ...
    WHEN OTHERS THEN
        RAISE NOTICE '⚠️ Warehouse index may already exist: %', SQLERRM;
END $$;
...
DO $$
BEGIN
    RAISE NOTICE '✅ Granted permissions on inventory table';
END $$;
...
DO $$
BEGIN
    RAISE NOTICE '✅ Created upsert_inventory_safe function';
END $$;

-- STEP 7: Create helper function to get stock level
-- =====================================================
DROP FUNCTION IF EXISTS get_stock_level(UUID, UUID, UUID);
...
DO $$
BEGIN
    RAISE NOTICE '✅ Created get_stock_level helper function';
END $$;
...
BEGIN
    ...
    ELSE
        RAISE WARNING '⚠️ Some checks did not meet expectations. Review the counts above.';
        IF constraint_count < 1 THEN
            RAISE WARNING '  Missing unique constraint';
        END IF;
        IF index_count < 2 THEN
            RAISE WARNING '  Missing indexes';
        END IF;
        IF policy_count < 4 THEN
            RAISE WARNING '  Missing RLS policies';
        END IF;
        IF function_count < 1 THEN
            RAISE WARNING '  Missing upsert function';
        END IF;
    END IF;
END $$;
...
BEGIN
    ...
    ELSE
        ...
        RAISE NOTICE '   org: %, product: %, branch: %', test_org_id, test_product_id, test_branch_id;
    END IF;
END $$;
