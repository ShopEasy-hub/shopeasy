...
export async function createOrganizationUser(orgId: string, userData: {
  ...
  try {
    ...
    if (!rpcError && rpcData) {
      ...
      // Check if manual steps are required (Free/Basic plan limitation)
      if (rpcData.manual_steps_required) {
        const instructions = rpcData.instructions;
        throw new Error(
          `⚠️ USER PROFILE CREATED - MANUAL AUTH SETUP REQUIRED\n\n` +
          `Due to your Supabase plan limitations, the user profile was created but login access requires manual setup.\n\n` +
          ...
          `6. Click "Create User"\n\n` +
          `Alternative: Deploy the Edge Function for automatic user creation.\n` +
          `File: /supabase/functions/create-organization-user/`
        );
      }
      ...
      if (rpcData.manual_steps) {
        ...
        throw new Error(
          ...
          `Alternative: Deploy the Edge Function 'create-organization-user' for automated user creation.`
        );
      }
    ...
    if (edgeError) {
      ...
      throw new Error(
        ...
        `Solutions:\n` +
        `1. Deploy the Edge Function: Run "supabase functions deploy create-organization-user"\n` +
        `2. Upgrade your Supabase plan to allow direct user creation\n` +
        `3. Manually create users in Supabase Dashboard (Authentication → Users)\n\n` +
    ...
    if (!edgeData || !edgeData.success) {
      ...
      throw new Error(edgeData?.error || 'Failed to create user');
    }
